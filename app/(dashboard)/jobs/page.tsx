...
// ★★★ FIX: Removed unused 'Calendar' and 'Clock' icons ★★★
import { MoreVertical, Edit, Trash2, X, AlertTriangle, Briefcase, MapPin, Building } from 'lucide-react';
...
// ★★★ FIX: Create a specific type for the form data to handle the string version of the deadline ★★★
type JobFormData = Omit<Partial<Job>, 'deadline'> & {
    deadline?: string;
};


// Skeleton loader for Job Cards (Unchanged)
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col transition-shadow hover:shadow-lg">
            <div className="flex-grow">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-slate-600 flex items-center gap-2 mt-1"><Building size={14} className="text-slate-400"/> {job.company}</p>
                    </div>
                    <div className="relative" ref={menuRef}>
                        <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 text-slate-500 rounded-full hover:bg-slate-100 transition-colors"><MoreVertical size={18}/></button>
                        {menuOpen && (
                            <div className="absolute right-0 top-full mt-2 w-36 bg-white rounded-xl shadow-xl border border-slate-200 z-10">
                                <button onClick={() => { onEdit(job); setMenuOpen(false); }} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-[#6D46C1] transition-colors rounded-t-xl"><Edit size={14}/> Edit</button>
                                <button onClick={() => { onDelete(job.id); setMenuOpen(false); }} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-slate-100 transition-colors rounded-b-xl"><Trash2 size={14}/> Delete</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between text-sm">
                    <p className="text-slate-500 flex items-center gap-2"><MapPin size={14} /> {job.location}</p>
                    <p className={`font-semibold px-2 py-1 rounded-full text-xs ${isExpired ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{isExpired ? 'Expired' : 'Active'}</p>
                </div>
                <p className="text-xs text-slate-400 mt-3">Deadline: {job.deadline?.toDate().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) ?? 'Not set'}</p>
            </div>
        </div>
    );
}
...
const EditJobModal = ({ job, isOpen, onClose, onSave }: { job: Job | null, isOpen: boolean, onClose: () => void, onSave: (id: string, data: Partial<Job>) => void }) => {
    // ★★★ FIX: Use the specific JobFormData type for the form state ★★★
    const [formData, setFormData] = useState<JobFormData>({});
    
    useEffect(() => {
        if (job) {
            // Convert Timestamp to a string format suitable for datetime-local input
            ...
            setFormData({ ...job, deadline: deadlineString });
        }
    }, [job]);
    ...
    const handleSave = () => {
        if (job) {
            // ★★★ FIX: Convert the deadline string back to a Timestamp before saving ★★★
            const { deadline, ...restOfData } = formData;
            const dataToSave: Partial<Job> = {
                ...restOfData,
                ...(deadline && { deadline: Timestamp.fromDate(new Date(deadline)) }),
            };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                {/* ★★★ FIX: Removed 'any' cast from value prop ★★★ */}
                                <input id="deadline" type="datetime-local" value={formData.deadline || ''} onChange={handleChange} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-md"/>
                            </div>
                            <div>
...
const AllJobsPage = () => {
    ...
    const handleDeleteJob = (id: string) => { setJobToDelete(id); setConfirmModalOpen(true); };
    ...
    const handleEditJob = (job: Job) => { setEditingJob(job); setEditModalOpen(true); };
        ...
        <>
            <Toaster position="top-right" reverseOrder={false} />
                <div className="w-full">
                    <div className="mb-8"><h1 className="text-3xl font-bold text-[#4c0e4c]">All Jobs</h1><p className="text-slate-500 mt-1">Manage all published job opportunities.</p></div>
                    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200">
                        {loading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{[...Array(3)].map((_, i) => <JobCardSkeleton key={i} />)}</div>) 
                        : jobs.length === 0 ? (<div className="text-center py-16"><Briefcase size={48} className="mx-auto text-slate-300" /><h3 className="mt-4 text-lg font-semibold text-slate-700">No Jobs Posted Yet</h3><p className="mt-1 text-slate-500">Create a new job to get started.</p></div>) 
                        : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{jobs.map(job => (<JobCard key={job.id} job={job} onDelete={handleDeleteJob} onEdit={handleEditJob} />))}</div>)}
                    </div>
                </div>
            <EditJobModal isOpen={isEditModalOpen} job={editingJob} onClose={() => setEditModalOpen(false)} onSave={handleUpdateJob} />
            <ConfirmationModal isOpen={isConfirmModalOpen} onClose={() => setConfirmModalOpen(false)} onConfirm={confirmDelete} title="Delete Job" message="Are you sure you want to delete this job post? This action cannot be undone."/>
        </>
    );
};
