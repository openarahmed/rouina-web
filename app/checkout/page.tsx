'use client'; // Eita client component, karon amra user interaction handle korbo

import { useSearchParams } from 'next/navigation';
import { useState } from 'react';
import { ShieldCheck } from 'lucide-react'; // Importing a nice icon for trust
...
export default function CheckoutPage() {
    ...
    // Payment er jonno hardcode kora amount
    const planDetails = {
        monthly: { amount: 25, currency: 'BDT' },
        yearly: { amount: 250, currency: 'BDT' }
    };
    
    // Type-safe key access for the plan
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (data.url) {
                // SSLCommerz payment page e redirect kora hocche
    ...
    // A loading or error state for when URL params are missing
    if (!uid || !plan) {
        return (
